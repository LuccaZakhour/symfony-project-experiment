...
function scrape_experiment_sections_files($db, $makeRequest, $clientId, $experiment_sections) {
    ...
    $data['experiment_sections_images'] = [];
    $i = 0;

    # first loop through experiments/sections/4051205/images
    foreach($experiment_sections as $experiment_section) {

        try {
            $experiment_section_images = $makeRequest('experiments/sections/' . $experiment_section['expJournalID'] . '/images');
        ...
        # insert into experiment_sections_images
        foreach ($experiment_section_images as $experiment_section_file) {
            ...
            $data['experiment_sections_images'][$experiment_section_file['experimentFileID']] = $experiment_section_file;
        }
        ...
        foreach ($experiment_section_images as $experiment_section_file) {
            ...
            # make request to experiments/sections/{expJournalID}/images/{experimentFileID}
            try {
                $fileContents = $makeRequest('experiments/sections/' . $expJournalID . '/images/' . $experimentFileID);
            } catch (Exception $e) {
                continue;
            }
            ...
            try {
                ...
                insertIntoTable($db, 'experiment_sections_images', [$experiment_section_file], 'experimentFileID');
